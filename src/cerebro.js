// El "cerebro" del bot: arma el mensaje para Claude, le da las herramientas
// para registrar pedidos y contactos, y devuelve la respuesta en texto.
...
export async function responder(telefono, texto) {
  ...
  charla.mensajes.push({ role: 'user', content: texto });
  guardarTurno(telefono, 'cliente', texto);

  let salida = '';

  // Hasta 5 vueltas: Claude puede pedir una herramienta y luego seguir escribiendo.
  for (let vuelta = 0; vuelta < 5; vuelta++) {
    const respuesta = await cliente.messages.create({
      ...
      tools: HERRAMIENTAS,
      messages: charla.mensajes.slice(-MAX_TURNOS),
    });

    charla.mensajes.push({ role: 'assistant', content: respuesta.content });
    ...
    const resultados = llamadas.map((llamada) => {
      const { resultado, aviso } = ejecutarHerramienta(llamada.name, llamada.input, telefono);
      if (aviso) charla.avisos.push(aviso);
      return { type: 'tool_result', tool_use_id: llamada.id, content: resultado };
    });

    charla.mensajes.push({ role: 'user', content: resultados });
  }
