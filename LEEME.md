...
3. Consiga una llave en <https://console.anthropic.com> → *API Keys* → *Create Key*
   y péguela en `.env`, en `ANTHROPIC_API_KEY=`.

   Se cobra por uso. Una conversación cuesta centavos de dólar. En esa misma
   consola puede ponerle un tope mensual de gasto.
...
```
ANTHROPIC_API_KEY=sk-ant-la-suya
...
```json
{"estado":"ok","claude":true,"registrosPersistentes":true,
 "dominiosPermitidos":["https://azzao.com","https://www.azzao.com"]}
```

Si `registrosPersistentes` dice `false`, el volumen no quedó bien montado y va a
perder los interesados. Revise el paso 2.3.
